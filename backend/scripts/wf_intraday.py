"""Gün içi walk-forward: ince dilim + dar adım uzayı + maker ücreti (3bps).
Grid'in gün-içi edge'i görülmemiş gelecekte tutuyor mu?"""
import os
import sys
...
# (interval, bars, train, test)
WIN = {"15m": (12000, 2880, 672), "1h": (8000, 1440, 336)}
RUNS = [
    ("SOL-USDT-SWAP", "15m", "total_return"),
    ("BTC-USDT-SWAP", "15m", "total_return"),
    ("SOL-USDT-SWAP", "1h", "total_return"),
    ("BTC-USDT-SWAP", "1h", "total_return"),
]

rec = Recorder("wf_intraday", "walkforward_matrix", label="gün-içi WF (maker 3bps)")
...
for sym, itv, obj in RUNS:
    ...
    try:
        r = gwf.run(symbol=sym, interval=itv, bars=bars, train_bars=tr, test_bars=te,
                    method="bayes", objective=obj, n_trials=30, fee_bps=3.0)
        s = r["summary"]
